...
export function TestRunsPage() {
  ...
  // The run draft is a single server-side record per project (one active
  // draft, no more local-vs-remote reconciliation needed).
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- clearing stale draft state on dependency change, not a derived-state sync
    if (draftDismissed || !projectId) { setActiveDraft(null); return undefined }
    let cancelled = false
    getRunDraft(projectId).then((draft) => { if (!cancelled) setActiveDraft(draft) })
    return () => { cancelled = true }
  }, [projectId, draftDismissed])
  ...
  // Guards so these effects only (re-)apply once per distinct query-string /
  // dropdown selection, not every time testCases/plans/requirements get a new
  // array reference (e.g. from an unrelated inline priority/status edit in
  // the case picker) - otherwise they'd silently clobber manual selection.
  const appliedSearchRef = useRef(null)
  const appliedPlanDropdownRef = useRef('')
  ...
  useEffect(() => {
    ...
    if (planIdParam) {
      ...
      if (plan) {
        appliedSearchRef.current = location.search
        appliedPlanDropdownRef.current = planIdParam
      ...
      // if the plan hasn't loaded yet, leave appliedSearchRef unset so this retries once plans arrives
    }
  ...
  useEffect(() => {
    if (!selectedTestPlanId) { appliedPlanDropdownRef.current = ''; return }
    if (appliedPlanDropdownRef.current === selectedTestPlanId) return
    const plan = plans.find((p) => p.id === selectedTestPlanId)
    if (!plan) return

    appliedPlanDropdownRef.current = selectedTestPlanId
    const scopeCases = getPlanTestCases(plan, requirements, testCases)
    // eslint-disable-next-line react-hooks/set-state-in-effect -- reacting to a user-driven dropdown change, not a derived-state sync
    setSelectedIds(scopeCases.map((tc) => tc.id))
    const dateStr = new Date().toLocaleDateString()
    setRunName(`${plan.name} – ${dateStr}`)
  ...
  const startRun = () => {
    ...
    saveRunDraft(projectId, {
      runName,
      build,
      testPlanId: selectedTestPlanId || null,
  ...
  const finishRun = async () => {
    ...
    if (selectedTestPlanId) {
      await linkRunToPlan(selectedTestPlanId, run.id)
    }

    await clearRunDraft(projectId)
    setDraftDismissed(true)

    setSavedRun({ ...run, passRate, failureModules: failingModules(executed) })
    setMode('complete')
  ...
  const handleRunBug = async (event) => {
  ...
  }, [mode, projectId, currentDraftId, runName, build, selectedTestPlanId, linkedRequirementId, selectedIds, currentIndex, results, bugsLogged, loggedBugCaseIds, loggedBugIds, startedAt])
